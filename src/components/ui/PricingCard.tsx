...
export default function PricingCard({ item }: PricingCardProps) {
  return (
    <div className="relative bg-white border border-gray-200 rounded-[23px] p-6 md:p-8 flex flex-col transition-all duration-500 hover:-translate-y-1 hover:scale-[1.02] hover:border-primary/40 hover:shadow-xl hover:shadow-primary/10">
      {/* Recommended Badge */}
      {item.recommended && (
      ...
      {/* Title */}
      <h3 className="text-2xl md:text-[28px] font-bold text-dark font-heading mb-1 pr-20">
        {item.name}
      </h3>
      ...
      <div className="flex items-end justify-between mt-auto pt-4">
        ...
        <div className="text-right">
          ...
          <div className="flex items-baseline gap-1 justify-end">
            <span className="text-3xl md:text-[40px] font-bold text-primary font-heading leading-none">
              {item.currentPrice}
            </span>
            {item.priceNote && (
              <span className="text-sm text-gray-400 font-medium">{item.priceNote}</span>
            )}
          </div>
        </div>
      </div>
